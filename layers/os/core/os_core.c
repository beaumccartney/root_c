internal String8 os_data_from_file_path(Arena *arena, String8 path)
{
	...
	OS_File file = os_file_open(OS_AccessFlag_Read, path);
...
{
	Assert(0 <= path.length);
	OS_File file = os_file_open(OS_AccessFlag_Write, path);
	Rng1S64 range = r1s64(0, data.length);
	os_file_write(file, range, data.buffer);
	B64 result = (B64)file.bits;
	os_file_close(file);
	return result;
}
...
internal B64 os_append_data_to_file_path(String8 path, String8 data)
{
	...
	if (data.length > 0)
	{
		OS_File file = os_file_open(OS_AccessFlag_Write|OS_AccessFlag_Append, path);
		FileProperties props = os_properties_from_file(file);
		Rng1S64 range = r1s64(props.size, props.size + data.length);
		os_file_write(file, range, data.buffer);
		result = (B64)file.bits;
		os_file_close(file);
	}
	return result;
}
internal String8 os_string_from_file_range(Arena *arena, OS_File file, Rng1S64 range)
{
