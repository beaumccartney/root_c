	...
	while (os_file_iter_next(scratch.arena, iter, &info))
	{
		...
		if (is_meta || is_c_lang)
		{
			...
			if (is_meta)
			{
				...
				{
					String8 lexeme = str8_substr(
						source,
						tokenize.tokens.tokens[m->tokens_ix].source_range
					);
					const local_persist
					...
					Vec2U64 pos = str8_pos_from_offset(source, token.source_range.min);
					printf(
						"%.*s(%llu, %llu): %s: %.*s\n",
						str8_varg(info.name),
						pos.line,
				...
				if (worst >= MD_MessageKind_Error)
				{
					// TODO: indicate that parsing won't happen
					continue;
				}

				MD_ParseResult parsed = md_parse_from_tokens(scratch.arena, tokenize.tokens);
				Unused(parsed);
			}
			else // is_c_lang
			{
